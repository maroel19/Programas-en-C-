...
void Consulta_Generallista(ldat list)
{
	...
	while(list!=NULL)
	{
		poscf(60,4+posi);printf("%i",list->codi1);
		poscf(65,4+posi);printf("%s",list->nomb1);
		poscf(80,4+posi);printf("%s",list->edad1);
		poscf(90,4+posi);printf("%s",list->estatu1);
		poscf(110,4+posi);printf("%c",list->estado1);
		list=list->sigu; 
		posi++;
	}
